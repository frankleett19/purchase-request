// ============================================================
// 三花生活館員購系統 — LINE Flex Message 範本（共用模組 / Cloudflare 版）
// 由 notify.js、monthly-summary.js 以 import 引用。
// 檔名底線開頭：Cloudflare Pages 不會把它變成路由，但可被 import。
// SITE 由各函式呼叫 buildMessage 時不需注入；預設網址讀不到 env 時用 fallback。
// 品牌主色：三花紅 #E8001D
// ============================================================

const BRAND = '#E8001D';
const SITE_FALLBACK = 'https://purchase-request.pages.dev';
...
// 1) 申請單送出確認 → 申請員工
function buildSubmitted(d, SITE) {
...
// 4) 待審核提醒 → 店長/督導
function buildPendingReview(d, SITE) {
...
// 5) 每月員購使用摘要 → 所有員工
function buildMonthlySummary(d, SITE) {
...
// site：傳入新平台網址（讀 env），未傳則用 fallback
export function buildMessage(type, data, site) {
  const fn = BUILDERS[type];
  if (!fn) throw new Error('unknown flex type: ' + type);
  const SITE = site || SITE_FALLBACK;
  const altTexts = {
    submitted: '您的員購申請已送出',
    approved: '您的員購申請已核准',
    rejected: '您的員購申請已退回',
  ...
  return { type: 'flex', altText: altTexts[type] || '員購系統通知', contents: fn(data || {}, SITE) };
}
